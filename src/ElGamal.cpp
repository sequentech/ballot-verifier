...
ElGamal::Ciphertext::Ciphertext(const mpz_class & aalpha, const mpz_class & abeta, const PublicKey & apk)
: alpha(aalpha), beta(abeta), pk(apk)
{
}

ElGamal::Plaintext::Plaintext(const mpz_class & am, const ElGamal::PublicKey & apk, const bool & encode_m)
: pk(apk)
{
	if(encode_m)
	{
		...
		mpz_class test;
		mpz_powm(test.get_mpz_t(), y.get_mpz_t(), pk.q.get_mpz_t(), pk.p.get_mpz_t()); // test = (y^q) mod p
...
ElGamal::Ciphertext ElGamal::encrypt(const ElGamal::PublicKey & pk, const ElGamal::Plaintext & plaintext, const mpz_class & rr)
{
	mpz_class r = rr;
	if(0 == plaintext.m)
	{
	...
	mpz_powm(alpha.get_mpz_t(), pk.g.get_mpz_t(), r.get_mpz_t(), pk.p.get_mpz_t() ); // alpha = (g^r) mod p
	mpz_powm(beta.get_mpz_t(), pk.y.get_mpz_t(), r.get_mpz_t(), pk.p.get_mpz_t()); // beta = ((y^r mod p) * m ) mod p
...
ElGamal::DLogProof ElGamal::Plaintext::proveKnowledge(const mpz_class & alpha, const mpz_class & randomness,
					 const ChallengeGenerator & challenge_generator)
{
	...
	mpz_mul(response.get_mpz_t(), randomness.get_mpz_t(), challenge.get_mpz_t()); 
...
bool ElGamal::Ciphertext::verifyPlaintextProof(const ElGamal::DLogProof & proof, const ElGamal::ChallengeGenerator & challenge_generator) const
{
	...
	mpz_powm( right.get_mpz_t(), alpha.get_mpz_t(), proof.challenge.get_mpz_t(), pk.p.get_mpz_t() );
	mpz_mul ( right.get_mpz_t(), right.get_mpz_t(), proof.commitment.a.get_mpz_t());
	mpz_mod ( right.get_mpz_t(), right.get_mpz_t(), pk.p.get_mpz_t() );
	
	//right == left ?
	//g^response == commitment * (g^t) ^ challenge == commitment * (alpha) ^ challenge ?
